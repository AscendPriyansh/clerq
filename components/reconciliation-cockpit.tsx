...
export function ReconciliationCockpit({ orgSlug, transactions, receipts, suggestions, suggestionCount, bankPagination, receiptPagination, suggestionPagination }: { orgSlug: string; transactions: WorkbenchRow[]; receipts: WorkbenchRow[]; suggestions: (Candidate & { bankName: string; receiptName: string })[]; suggestionCount: number; bankPagination: ReactNode; receiptPagination: ReactNode; suggestionPagination: ReactNode }) {
  ...
  const action = (fn: () => Promise<{ error?: string }>, success: string) => startTransition(async () => {
    try { const result = await fn(); setMessage(result.error ?? success); if (!result.error) state.clear(orgSlug); }
    catch { setMessage("The action failed. Refresh and try again."); }
  });
  return <div className="space-y-5" aria-busy={pending}>{pending && <p role="status">Updating reconciliation…</p>}<Button disabled={pending} onClick={() => startTransition(async () => { const result = await reconcileNow(orgSlug); setMessage("error" in result ? result.error ?? "Reconciliation failed." : `${result.exactMatches} exact matches; ${result.suggestions} suggestions.`); })}>Run reconciliation</Button>
    ...
    <section className="space-y-3"><h3 className="text-lg font-semibold">Suggested matches ({suggestionCount})</h3>{!suggestions.length && <p>No suggestions to review.</p>}{suggestions.map(pair => <div key={`${pair.receiptId}:${pair.bankTransactionId}`} className="flex flex-wrap items-center justify-between gap-3 rounded border p-3"><div><p>{pair.bankName} ↔ {pair.receiptName}</p><p className="text-sm">Amount difference {pair.amountDifference}; date difference {pair.dateDifference} days; vendor score {Math.round(pair.confidenceScore * 100)}%</p></div><div className="flex gap-2"><Button disabled={pending} onClick={() => action(() => matchRecords(orgSlug, pair.receiptId, pair.bankTransactionId, false), "Suggestion confirmed.")}>Confirm</Button><Button variant="outline" disabled={pending} onClick={() => action(() => dismissSuggestion(orgSlug, pair.receiptId, pair.bankTransactionId), "Suggestion dismissed.")}>Dismiss</Button></div></div>)}{suggestionPagination}</section>
  </div>;
}
