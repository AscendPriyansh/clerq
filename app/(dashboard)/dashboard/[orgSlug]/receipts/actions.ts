...
export async function saveReceiptReview(orgSlug: string, receiptId: string, form: FormData) {
  try {
    ...
    const job = await prisma.$transaction(async tx => {
      ...
      return tx.ingestionJob.create({ data: { key: `review:${randomUUID()}`, type: "RECONCILE", payload: { orgId: organization.id } } });
    });
...
export async function retryReceipt(orgSlug: string, receiptId: string) {
  try {
    ...
    const queued = await prisma.$transaction(async tx => {
      ...
      return tx.ingestionJob.upsert({ where: { key: `parse:${receiptId}` }, create: { key: `parse:${receiptId}`, type: "RECEIPT_PARSE", payload: { receiptId, orgId: organization.id } }, update: { status: "PENDING", attempts: 0, lockedAt: null, availableAt: new Date(), lastError: null } });
    });
    await dispatchJob(queued.id);
    revalidatePath(`/dashboard/${orgSlug}`, "layout");
    return { success: true };
