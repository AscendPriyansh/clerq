...
import { MAX_FILE_BYTES, MIME_EXTENSIONS, readLimitedBody } from "@/lib/files";
import { AppError, errorResponse } from "@/lib/errors";

export async function POST(request: Request) {
  try {
    const input = z.object({ orgSlug: z.string(), mimeType: z.enum(["application/pdf", "image/png", "image/jpeg", "image/webp"]), size: z.number().int().min(1).max(MAX_FILE_BYTES) }).parse(JSON.parse((await readLimitedBody(request, 4096)).toString()));
